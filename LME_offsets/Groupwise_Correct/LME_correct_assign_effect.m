function Bias = LME_correct_assign_effect(lme,P)
    ...
    if P.do_random == 0
        ...
        Bias.bias_fixed  = lme.out_rnd.bias_fixed_random(en,:)';

        if P.do_region == 1
            Bias.bias_region = lme.out_rnd.bias_region_rnd(:,:,en);
        end

        if P.do_season == 1
            Bias.bias_season = lme.out_rnd.bias_season_rnd(:,:,en);
        end

        if P.do_decade == 1
            Bias.bias_decade = lme.out_rnd.bias_decade_rnd(:,:,en);
        end

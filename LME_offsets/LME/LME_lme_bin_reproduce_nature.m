function [BINNED,W_X] = LME_lme_bin_reproduce_nature(P)
    ...
    file_save = [dir_save,'SUM_pairs_',P.save_bin,'.mat'];
    fid = fopen(file_save);
    ...
    if fid <= 0,
        ...
        % Load previous pairs
        % *****************************************************************
        dir_nat = '/Volumes/Untitled/01_Research/03_DATA/HvdSST/For public/Check_points/';
        file_nat = [dir_nat,'SUM_HM_SST_Bucket_Screen_Pairs_c_once_',...
            '1850_2014_NpD_1_rmdup_0_rmsml_0_fewer_first_0.mat'];
        a = matfile(file_nat);
        yr_nature = a.DATA(1,:);
        mon_nature = a.DATA(2,:);
        uid_1_nature = a.DATA(7,:);
        uid_2_nature = a.DATA(7+21,:);
        lcl_1_nature = a.DATA(5,:);
        lcl_2_nature = a.DATA(5+21,:);
        ...
        for yr = P.yr_list
            for mon = P.mon_list
                ...
                % DATA = [DATA DATA_save];
                ll = yr_nature == yr & mon_nature == mon;
                if nnz(ll) > 0,
                    DATA_save = [uid_1_nature(ll);  uid_2_nature(ll)];
    ...
    clear('yr_nature','mon_nature','uid_1_nature','uid_2_nature',...
          'lcl_1_nature','lcl_2_nature','ll')
    ...
    l_rp = all([P1.DCK P1.C0_SI_4'] == [P2.DCK P2.C0_SI_4'],2);
    
    if 0,   % New methods each group should have at least P.key pairs
        [~,~,L] = unique([[P1.DCK P1.C0_SI_4'];[P2.DCK P2.C0_SI_4']],'rows');
        l_sm = ismember(L,find(hist(L,1:1:max(L)) <= P.key));
        l_sm = any([l_sm(1:numel(l_sm)/2)  l_sm(numel(l_sm)/2+1:end)],2);
        clear('L');
    else    % To be consistent with script used in the nature paper
        list_large_group = HM_pair_function_large_groups(1,P.varname,P.method,0);
        l1 = ismember(P1.DCK,list_large_group,'rows');
        l2 = ismember(P2.DCK,list_large_group,'rows');
        l_sm = ~l1 | ~l2;
    end
    ...
    N_pairs = nnz(~l_rm);
    clear('l_rp','l_sm','l_rm','DATA','l1','l2')
